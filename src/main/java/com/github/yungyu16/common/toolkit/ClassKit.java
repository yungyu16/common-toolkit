...
public class ClassKit {
    ...
    public static boolean isInnerClass(Class clazz) {
        return clazz == null ? false : StringKit.contains(clazz.getName(), '$');
    }

    public static boolean isAssignable(Class[] classes, Class[] fromClasses) {
        if (!ArrayKit.isSameLength(fromClasses, classes)) {
            return false;
        } else {
            if (fromClasses == null) {
                fromClasses = ArrayKit.EMPTY_CLASS_ARRAY;
            }

            if (classes == null) {
                classes = ArrayKit.EMPTY_CLASS_ARRAY;
            }
        ...
        private TypeInfo(Class type) {
            ...
            if (this.dimension > 0) {
                ...
                if (superComponentType != null) {
                    componentInterface = ClassKit
                            .getArrayClass(superComponentType, this.dimension);
                    this.superclasses.add(componentInterface);
                    this.superclasses
                            .addAll(ClassKit.getTypeInfo(componentInterface).superclasses);
                } else {
                    for (int i = this.dimension - 1; i >= 0; --i) {
                        this.superclasses.add(ClassKit.getArrayClass(Object.class, i));
                    }
                }
            } else {
                ...
                if (superComponentType != null) {
                    ...
                    this.superclasses
                            .addAll(ClassKit.getTypeInfo(superComponentType).superclasses);
                }
            }
            ...
            if (this.dimension == 0) {
                ...
                for (int i = 0; i < typeInterfaces.length; ++i) {
                    ...
                    set.addAll(ClassKit.getTypeInfo(interfaceClass).interfaces);
                }
                ...
                Iterator i = ClassKit.getTypeInfo(componentType).interfaces.iterator();

                while (i.hasNext()) {
                    componentInterface = (Class) i.next();
                    this.interfaces
                            .add(ClassKit.getArrayClass(componentInterface, this.dimension));
                }
            }
